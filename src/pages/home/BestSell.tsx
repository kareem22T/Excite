...
const BestSellingProductsSlider: React.FC = () => {
  ...
  const fetchProducts = async ()  => {
    try {
      const response = await api.get('https://excite.techno-era.co/en/api/oscar/products/?page=1&page_size=10&ordering=-stockrecords__price,title');
  ...
  const addProductTo = async (id: number) => {
    dispatch(addProductToWishlist({ id: id, callback: fetchProducts }));
  }  
          ...
          product && (
            <SwiperSlide>
                <div className="img">
                    ...
                    <button onClick={() => addProductTo(product.id)} className={"add_to_wishlist"  + (product.in_wishlist ? " active" : "")}>
